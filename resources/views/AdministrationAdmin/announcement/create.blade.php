...
@section('content')
    <form action="{{ route('administrationadmin.announcement.store') }}" method="POST">
        @csrf
        <div class="container-xxl flex-grow-1 container-p-y">
            <h5 class="fw-bold py-3 mb-4">
                <span class="text-muted fw-light"><a href="{{ route('administrationadmin.announcement.index') }}">Daftar Pengumuman</a> /
                </span>
                Tambah Pengumuman
            </h5>
            <div class="row">
                                ...
                                <select class="form-select select2 @error('target_id') is-invalid @enderror" id="target_id"
                                    ...
                                    @foreach ($targets as $target)
                                        <option value="{{ $target->id }}"
                                            {{ old('target_id') == $target->id ? 'selected' : '' }}>{{ $target->name }}
                                        </option>
                                    @endforeach
                                </select>
