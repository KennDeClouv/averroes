                    ...
                    <tbody>
                        @foreach ($studentRegistrants as $studentRegistrant)
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('administrationadmin.studentregistrant.show', $studentRegistrant->id) }}" data-bs-toggle="tooltip"
                                        data-bs-placement="top" title="Detail Santri" class="btn btn-info "><i
                                            class="fa-solid fa-eye fs-6"></i></a>
                                    <a href="{{ route('administrationadmin.studentregistrant.edit', $studentRegistrant->id) }}" data-bs-toggle="tooltip"
                                        data-bs-placement="top" title="Edit Santri" class="btn btn-warning "><i
                                            class="fa-solid fa-edit fs-6"></i></a>
                                    <x-delete :route="route('administrationadmin.studentregistrant.destroy', $studentRegistrant->id)" :message="'Apakah anda yakin ingin menghapus user calon santri ' . $studentRegistrant->name . '?'" :title="'Hapus User Calon Santri'" />
                                </td>
                            </tr>
                        @endforeach
                    </tbody>
